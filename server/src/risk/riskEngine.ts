...
import type { LoadedPolicy, ThresholdAction } from "../policy/policyEngine.js";
import type { HardGateResult } from "../detectors/index.js";
...
const ACTION_SEVERITY: Record<ThresholdAction, number> = {
  ALLOW: 0,
  EDIT_CLARIFY: 1,
  REGENERATE: 2,
  HUMAN_REVIEW: 3,
...
const ACTION_RISK_LEVEL: Record<ThresholdAction, RiskLevel> = {
  ALLOW: "low",
  EDIT_CLARIFY: "medium",
  REGENERATE: "medium",
  HUMAN_REVIEW: "high",
...
function metricValue(metrics: MetricsBundle, name: string): number | undefined {
  switch (name) {
    case "pls":
      return metrics.pls;
    case "shs":
...
function enforceAllowedActions(action: ThresholdAction, allowed: ThresholdAction[]): ThresholdAction {
...
export function evaluateRisk(
  ...
  let bestAction: ThresholdAction = "ALLOW";
